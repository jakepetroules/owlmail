...
void UpdateDialog::checkReplyFinished(QNetworkReply *reply)
{
    ...
    if (this->m_newVersion.isValid() && !this->m_downloadUrl.isEmpty() && !this->m_checksum.isEmpty())
    {
        ...
        {
            this->setError(tr("Your version of KSC Email Tracker appears to be newer than the latest available version. Please contact <a href=\"http://www.petroules.com/\">Petroules</a> for further assistance."));
        }
    }
    else
    {
...
{
    this->setError(QString(tr("A network error was encountered when attempting to determine the latest version of KSC Email Tracker. The error returned was: %1")).arg(error));
}
...
void UpdateDialog::setAlreadyLatest(const QVersion &version)
{
    ...
    this->ui->updateCheckCompletedLabel->setText(QString(tr("Congratulations, you are running the latest version of KSC Email Tracker (<b>%1</b>)! There is no need to update.")).arg(version.toString()));
}
...
void UpdateDialog::acceptUpgrade()
{
    ...
    QFile tempFile(TrackerPreferences::instance().updateInstallerPath());
...
{
    this->ui->downloadingLabel->setText(QString(tr("Downloading KSC Email Tracker version <b>%1</b> from %2")).arg(this->m_newVersion.toString()).arg(this->m_downloadUrl.toString()));
...
void UpdateDialog::downloadReplyFinished(QNetworkReply *reply)
{
    ...
    if (temp.open())
    {
        TrackerPreferences::instance().setUpdateInstallerPath((this->m_file = temp.fileName()));
