...
; Constants
	...
	!define PATH_RES "..\..\res"
	!define PATH_APPRES "..\..\${PRODUCT_UNIXNAME}\res"
...
; Installer Sections
	...
	SectionIn RO
		...
		SetOutPath "$INSTDIR"
		File "${APP_BIN}\kscemailtracker.exe"
		...
		File "${QT_BIN}\QtWebKit4.dll"
		File "${QT_BIN}\QtXmlPatterns4.dll"
		File "${QT_BIN}\phonon4.dll"
		...
		File "${QT_PLUGINS}\imageformats\qgif4.dll"
		File "${QT_PLUGINS}\imageformats\qjpeg4.dll"
		...
		CreateShortCut "$SMPROGRAMS\$StartMenuFolder\$(LANG_CaptionMainApp).lnk" "$INSTDIR\kscemailtracker.exe" "" "$INSTDIR\kscemailtracker.exe" 0 SW_SHOWNORMAL "" $(LANG_CaptionMainApp)
	...
	Section -Post
		...
		WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\kscemailtracker.exe"
		WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
		WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninstall.exe"
		WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\kscemailtracker.exe"
...
; Uninstaller Section
	...
	!macro un.RemoveUninstaller
		...
		IfFileExists "$INSTDIR\kscemailtracker.exe" LeaveUninstaller +1
	...
	Section "un.${PRODUCT_NAME} ${PRODUCT_VERSION}" UnSecCoreFiles
		...
		Delete "$INSTDIR\kscemailtracker.exe"
